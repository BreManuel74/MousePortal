...
class LickAnalysis:
    ...
    def get_session_quarters(self):
        ...
        # Align lengths
        min_len = min(len(reward_times), len(reward_change_times_flat))
        reward_times = reward_times[:min_len]
        reward_change_times_flat = reward_change_times_flat[:min_len]
        ...
        for i in range(4):
            ...
            # Find reward times and change times in this quarter
            mask = (reward_times >= start) & (reward_times < end)
            reward_times_q = reward_times[mask]
            reward_change_times_q = reward_change_times_flat[mask]

            # Calculate reward_delays for this quarter
            reward_delays_q = [t_reward - t_change for t_change, t_reward in zip(reward_change_times_q, reward_times_q)]
    ...
    def compute_metrics_for_window(self, start_time, end_time):
        ...
        # Prepare arrays for the whole session, then filter to window
        reward_texture_change_time = self.prepare_arrays()
        reward_change_times_flat = reward_texture_change_time.flatten()
        reward_change_times_flat = pd.to_numeric(reward_change_times_flat, errors='coerce')
        reward_change_times_flat = reward_change_times_flat[~np.isnan(reward_change_times_flat)]
        reward_change_times_flat = reward_change_times_flat[
            (reward_change_times_flat >= start_time) & (reward_change_times_flat < end_time)
        ]
        ...
        #print(f"DEBUG: reward_times (window {start_time}-{end_time}): {reward_times}")
        #print(f"DEBUG: reward_change_times_flat (window {start_time}-{end_time}): {reward_change_times_flat}")

        # Calculate reward_delays for this window
        reward_delays = [t_reward - t_change for t_change, t_reward in zip(reward_change_times_flat, reward_times)]

        licks_before_reward = [
            int(np.sum((lick_bout_times_window >= t_change) & (lick_bout_times_window < t_reward)))
            for t_change, t_reward in zip(reward_change_times_flat, reward_times)
        ]
        ...
        licks_before_reward_zone = [
            ...
            for t_change, delay in zip(reward_change_times_flat, reward_delays)
        ]
        ...
        licks_after_reward = [
            ...
            for t_reward, delay in zip(reward_times, reward_delays)
        ]
