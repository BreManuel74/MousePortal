...
class Corridor:
    ...
    def recycle_segment(self, direction: str) -> None:
        ...
        # After recycling, ensure a segment covers the camera's Y position (assume camera at y=0)
        camera_y = 0.0  # Or self.base.camera_position if variable
        covered = False
        for seg in self.left_segments:
            seg_y = seg.getY()
            if seg_y <= camera_y < seg_y + self.segment_length:
                covered = True
                break
        if not covered:
            # If not covered, move the farthest segment to cover the camera
            farthest_seg = max(self.left_segments, key=lambda s: abs(s.getY() - camera_y))
            farthest_seg.setY(camera_y - (self.segment_length / 2))
            # Optionally reapply texture
            self.apply_texture(farthest_seg, farthest_seg.getTexture().getFilename())
